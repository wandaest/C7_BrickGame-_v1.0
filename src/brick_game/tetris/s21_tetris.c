...
void userInput(UserAction_t sig, bool hold) {
  ...
  if (act)
    act(&params, info);  // Передаём адрес params
...
/**
 * @brief Преобразует код пользовательского ввода в действие.
 *
 * Эта функция интерпретирует код пользовательского ввода (например, нажатие
 * клавиш) и возвращает соответствующее действие, которое будет использоваться в
 * игре.
 *
 * @param user_input Код пользовательского ввода.
 * @return UserAction_t Тип действия, соответствующий пользовательскому вводу.
...
void shifting(GameParameters *params, GameInfo_t *GameInfo) {
  if (GameInfo->pause == 1) {
    GameInfo->pause = 0;  // Изменение: Убран timeout, так как он зависит от ncurses
  }
...
void rotate(GameParameters *params, GameInfo_t *GameInfo) {
  ...
  remove_figure_from_field(&GameInfo->field, *params->current_figure);
  if (!check_collision(*GameInfo, buffer) && !check_collide_for_rotation(buffer, *GameInfo)) {
 ...
 *
 * Функция инициализирует структуру GameInfo, создаёт фигуры и запускает
 * основной игровой цикл. В цикле обрабатываются пользовательские вводы и
 * изменяется состояние игры. Игровой цикл продолжается до тех пор, пока не
 * будет достигнуто состояние GAMEOVER или EXIT_STATE. В конце освобождаются
 * ресурсы, связанные с GameInfo.
...
void run_game_loop() {
  ...
  while (break_flag) {
    if (state == GAMEOVER || state == EXIT_STATE)
      break_flag = false;
    UserInput(get_user_action(signal), &state, &next_figure, &current_figure,
              &GameInfo);
...
// Изменение: Добавлена функция init_game для инициализации состояния игры
/*void init_game(GameInfo_t *GameInfo, figure *current_figure, figure *next_figure,  GameState *state) {
  initialize_GameInfo_t(GameInfo);
  initialize_next_figure(next_figure);
  spawn_new_figure(*next_figure, current_figure);
  *state = START;
}*/
// Изменение: init_game использует reset_game
void init_game(GameInfo_t *GameInfo_out, figure *next_figure_out, figure *current_figure_out, GameState *state_out) {
  reset_game();
...
// Изменение: Добавлена функция update_game для пошагового обновления состояния без ncurses
/*void update_game(GameInfo_t *GameInfo, figure *next_figure, figure *current_figure, GameState *state, UserAction_t sig, double elapsed_time) {
  GameParameters params = {state, next_figure, current_figure, NULL};
  ...
  if (*state == MOVING && !GameInfo->pause) {
    int drop_interval = 500 - GameInfo->speed * 40;  // Интервал падения в миллисекундах
    static double accumulated_time = 0;  // Изменение: Статическая переменная для накопления времени
 ...
 *
 * Функция открывает файл "high_score.txt", считывает рекордный результат и
 ...
 *
 * Функция открывает файл "high_score.txt" для записи и сохраняет текущий
...
void update_speed(GameInfo_t *GameInfo) {
  if ((GameInfo->score > SCORE_PER_LEVEL * GameInfo->level) &&
    ...
    if (GameInfo->level > MAX_GAME_LEVEL)
      GameInfo->level = MAX_GAME_LEVEL;
