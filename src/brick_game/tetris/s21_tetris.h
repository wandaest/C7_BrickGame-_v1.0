...
void init_game(GameInfo_t *GameInfo, figure *next_figure, figure *current_figure, GameState *state);
...
void free_matrix_memory(int ***matrix, int row); // Освобождение памяти матрицы.
...
void spawn(GameParameters *params, GameInfo_t *GameInfo); // Спавн новой фигуры.
...
// Прототипы функций для обработки пользовательского ввода.
UserAction_t
get_user_action(int user_input); // Получение действия пользователя по вводу.
void UserInput(UserAction_t sig, GameState *state, figure *next_figure,
               figure *current_figure,
               GameInfo_t *GameInfo); // Обработка ввода пользователя.

void init_colors(void);
void display_overlay(void); // Отображение наложения на экран.
void display_rectangle(int top_y, int bottom_y, int left_x,
                       int right_x); // Отображение прямоугольника.
void display_next_figure(GameInfo_t GameInfo); // Отображение следующей фигуры.
void display_game_field(GameInfo_t GameInfo); // Отображение игрового поля.
void display_game_stats(GameInfo_t GameInfo); // Отображение статистики игры.
void display_current_game_state(
    GameInfo_t GameInfo); // Отображение текущего состояния игры.

// Функции GUI (будут вынесены в отдельный модуль)
void init_gui(void);  // Инициализация GUI
